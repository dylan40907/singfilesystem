// supabase/functions/admin-delete-campus-admin/index.ts
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { campus_admin_id } = (await req.json().catch(() => ({}))) as { campus_admin_id?: string };
    const targetId = (campus_admin_id ?? "").trim();
    if (!targetId) return json({ error: "campus_admin_id required" }, 400);
    ...
    // Verify actor is a TRUE admin (campus admins can't delete other campus admins)
    ...
    // Only allow deleting campus_admins
    ...
    if (targetProfile.role !== "campus_admin") {
      return json({ error: "Refusing to delete a non-campus-admin." }, 400);
    }
    ...
    if (delErr) return json({ error: "Auth delete error: " + delErr.message }, 400);

    return json({ ok: true });
  } catch (e) {
    console.error("admin-delete-campus-admin error", e);
